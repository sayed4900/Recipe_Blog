...
exports.homepage = async (req, res, next) => {
  try {
    ...
    res.status(500).send({ meessage: error || 'ERROR OCCURED' });
  }
};
...
// Get all recipes with specific category
exports.getRecipesOnCategory = async (req, res, next) => {
  try {
    const specificCategory = req.params.category;
    const limitNumber = 20;
    const recipes = await Recipe.find({ category: req.params.category }).limit(
      limitNumber
    );
...
exports.searchRecipe = async (req, res) => {
  ...
  try {
    ...
    // res.json(recipe);
    res.render('search', {
      title: `Cooking Blog-Search-> ${recipe.name}`,
      recipe,
    });
  } catch (error) {
    res.status(500).send({ meessage: error || 'ERROR OCCURED' });
...
// GET random recipe
exports.exploreRandomRecipe = async (req, res, next) => {
  try {
    let count = await Recipe.find().countDocuments();
    let random = Math.floor(Math.random() * count);
    let recipe = await Recipe.findOne().skip(random).exec();

    res.render('exploreRandomRecipe', {
